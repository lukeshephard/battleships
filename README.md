# Battleships
🔗 **Live site**: https://battleships.shephardluke.co.uk
...
1. **Clone the repo**
    ...
    ```
    git clone https://github.com/lukeshephard/battleships.git
    cd battleships
    ```
...
## 🕹️ Playing
### Setup
Press the play button which will start the game setup. You have to place your 5 ships where you want on the grid by first clicking the start square, then you will see a preview of where your ship will be if you move the mouse around. Clicking will place the previewed ship in that place.
...
#### Your turn
When the text in the middle says its your turn, you can click a square in the enemy's (top) grid to guess where you think one of their ships is. Once you click the square it will either turn white (miss) or red (hit their ship).

#### The enemy's (CPU) turn
Once you have guessed a square, the enemy will try to guess one of your squares. You will see in your bottom grid which square they shot and if it hit your ship (red square) or if it was a miss (white square).
...
|------------|------------------------------------------------------------------|----------------------------------------------------------------------------|-----------|------------|
|Random mode |No hits on unsunk ships                                           |Chooses a random square that has not been fired at before.                  |Destroy    |Random      |
|Destroy mode|Hit(s) on unsunk ship(s) but unknown ship direction               |Chooses a random square around the hit(s) that has not been fired at before.|Line       |Destroy                                                                                         |
|Line mode   |Adjacent hits with empty square in the same direction either side|Chooses a random square of the two sides that has not been fired at before. |Line       |If at least 1 empty square on a side then line, otherwise no empty square on either side is destroy*|

*No empty square either side of a line can only occur if the player has ships side by side in parallel. The switch to destory mode will allow the CPU to not get stuck and hit those ships in the other axis.
